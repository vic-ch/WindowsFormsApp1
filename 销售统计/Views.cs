    ...
    public static class Views
    {
        ...
        public static string Thisday
        {
            ...
            set => thisday = 
                ...
                "AND(SELECT 进货日 FROM  " + value + "  WHERE 商品编号 = A.商品编号)=(SELECT DATEPART(DD, GETDATE()))";
        }
        ...
        public static string Thisseason 
        {
            ...
            set => thisseason = 
                ...
                "FROM "+value+" AS A " +
                "WHERE (SELECT " + (value == "sell" ? "销售" : "进货") + "年 FROM  " + value + "  WHERE 商品编号=A.商品编号)=(SELECT DATEPART(YY, GETDATE())) " +
                "AND(SELECT DATEPART(QQ, CONCAT((SELECT " + (value == "sell" ? "销售" : "进货") + "年 FROM  " + value + "  WHERE 商品编号 = A.商品编号),'-',(SELECT TOP 1 " + (value == "sell" ? "销售" : "进货") + "月 FROM  " + value + "  WHERE 商品编号=A.商品编号),'-',(SELECT TOP 1 进货日 FROM  " + value + "  WHERE 商品编号=A.商品编号))))  = (SELECT DATEPART(QQ, GETDATE()))";
        }
        ...
        { 
            ...
            set => thisseason_group =
                ...
                "AND(SELECT DATEPART(QQ, CONCAT((SELECT 进货年 FROM  " + value + "  WHERE 商品编号 = A.商品编号),'-',(SELECT 进货月 FROM  " + value + "  WHERE 商品编号=A.商品编号),'-',(SELECT 进货日 FROM  " + value + "  WHERE 商品编号=A.商品编号))))  = (SELECT DATEPART(QQ, GETDATE()))" +
                "GROUP BY 生产厂商";
                }
