...
{
    public partial class Checkretreat : Form
    {
        public Checkretreat()
        {
            InitializeComponent();
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    conn.Open();
                    using (SqlCommand cmd = new SqlCommand(Views.Retreat, conn))
                    {
                        using (SqlDataAdapter adapter = new SqlDataAdapter(cmd))
                        {
                            int count = adapter.Fill(ds, "退货货物");
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                if (textBox1.Text.Length > 0)
                {
                    ...
                    dt = ds.Tables["退货货物"].Clone();
                    DataRow[] dataRow = ds.Tables["退货货物"].Select("商品名='" + textBox1.Text + "' AND 型号='"+textBox2.Text+"'");
        ...
        {
            dataGridView1.DataSource = ds.Tables["退货货物"];
